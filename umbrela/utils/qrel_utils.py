...
def generate_holes(qrel, judge_cat=[0, 1, 2, 3], exception_qid=[]):
    qrel_data = get_qrels(qrel)
    holes = []
    gts = []
...
def get_index_reader(qrel):
    # Index reader
    if qrel in ["dl19-passage", "dl20-passage"] or 'dl19' in qrel or 'dl20' in qrel:
...
def get_passage_msv2(pid):
    # print('Reading passage from MS MARCO v2, pid:', pid)
...
def get_passage_wrapper(qrel, doc_id):
    ...
    elif qrel in ["dl21-passage", "dl22-passage", "dl23-passage"] or 'dl21' in qrel or 'dl22' in qrel or 'dl23' in qrel:
...
def prepare_query_passage(qid_docid_list, qrel):
    query_mappings = get_query_mappings(qrel)
    query_passage = []
    for sample in tqdm.tqdm(qid_docid_list):
